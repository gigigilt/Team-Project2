...
CSV_NAME = "google_with_cluster3_aha.csv"
...
def load_data():
    ...
    with zipfile.ZipFile(io.BytesIO(resp.content)) as zf:
        with zf.open(CSV_NAME) as f_bin:
            # 1) C 엔진 시도 (빠르고 메모리 효율적)
            try:
                df = pd.read_csv(
                    f_bin,
                    engine="c",             # ✅ c 엔진
                    on_bad_lines="skip",    # 필요 시 "error"/"warn"로 변경
                    encoding_errors="ignore"
                )
            except Exception:
                # 2) python 엔진 폴백 (※ low_memory 넣지 않음!)
                f_bin.seek(0)
                df = pd.read_csv(
                    f_bin,
                    engine="python",        # ✅ python 엔진
                    on_bad_lines="skip",
                    encoding_errors="ignore"
                )

    # 긴 숫자 ID는 문자열로 고정 (집계 변동 방지)
    if "fullVisitorId" in df.columns:
        df["fullVisitorId"] = df["fullVisitorId"].astype("string")
    if "visitStartTime" in df.columns:
        df["visitStartTime"] = df["visitStartTime"].astype("string")
    return df
