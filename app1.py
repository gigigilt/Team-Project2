...
# 1) 리포에 올라온 ZIP 상대경로 (앱 루트 기준)
ZIP_LOCAL_PATH = Path("./google_with_cluster3_aha.zip")   # ✅ 리포에 올린 파일명 그대로
# 2) 혹시 로컬에 없을 때만 RAW URL로 폴백
ZIP_URL = "https://raw.githubusercontent.com/gigigilt/Team-Project2/main/google_with_cluster3_aha.zip"

@st.cache_data(show_spinner=True)
def load_data():
    # --- 0) ZIP 바이트 확보: 로컬 우선, 없으면 URL 폴백
    if ZIP_LOCAL_PATH.exists():
        st.info(f"로컬 ZIP에서 로드: {ZIP_LOCAL_PATH}")
        with open(ZIP_LOCAL_PATH, "rb") as f:
            zip_bytes = f.read()
    else:
        st.info("GitHub RAW에서 ZIP 다운로드 중…")
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
        ...
        if not names:
            raise RuntimeError(f"ZIP 안에 CSV가 없습니다. 포함된 파일: {zf.namelist()}")
        # 가장 큰 CSV 선택
        csv_name = max(names, key=lambda n: zf.getinfo(n).file_size)
        st.caption(f"CSV 선택됨: {csv_name}")

        with zf.open(csv_name) as f_bin:
            # C 엔진 → 실패 시 Python 엔진
            try:
                df = pd.read_csv(f_bin, engine="c", on_bad_lines="skip", encoding_errors="ignore")
            except Exception:
                f_bin.seek(0)
                df = pd.read_csv(f_bin, engine="python", on_bad_lines="skip", encoding_errors="ignore")

    # --- 2) 식별자 컬럼은 문자열로 강제
    for col in ("fullVisitorId", "sessionId", "visitStartTime", "clientId"):
