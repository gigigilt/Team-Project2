...
if d_stick.empty:
    ...
    fig_dau_wau = px.line(daily, x="date", y="DAU/WAU", title="고착도 (DAU/WAU, 일별)", markers=True)
    fig_dau_wau.update_yaxes(rangemode="tozero")
    st.plotly_chart(fig_dau_wau, width="stretch", key=ukey("stick-dau-wau-daily"))
    ...
    if weekly.empty or monthly.empty:
        st.info("주/월 단위 고착도 계산에 필요한 데이터가 부족합니다.")
    else:
        ...
        fig_wau_mau = px.line(wk.sort_values("week"), x="week", y="WAU/MAU", title="고착도 (WAU/MAU, 주별)", markers=True)
        fig_wau_mau.update_yaxes(rangemode="tozero")
        st.plotly_chart(fig_wau_mau, width="stretch", key=ukey("stick-wau-mau-weekly"))
