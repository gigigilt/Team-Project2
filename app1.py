...
# pandas DatetimeTZ dtype 안전 체크용 (버전 호환)
try:
    from pandas.core.dtypes.dtypes import DatetimeTZDtype as _DatetimeTZDtype  # 내부경로지만 가장 호환 좋음
except Exception:  # fallback
    _DatetimeTZDtype = type("DTZ_PLACEHOLDER", (), {})
...
def load_data() -> pd.DataFrame:
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(r.content)) as z:
            # zip 안 첫 파일 사용 (파일명이 고정이면 그대로 써도 됨)
            fname = z.namelist()[0]
...
def ensure_dt(s: pd.Series) -> pd.Series:
    ...
    # is_datetime64tz_dtype 경고 대체
    if isinstance(out.dtype, _DatetimeTZDtype):
        try:
...
with left:
    # dataframe은 width 문자열을 지원하지 않으므로 use_container_width만 사용
    st.dataframe(
...
if d_stick.empty:
    ...
    # ── 공통: 일/주/월 집계 준비(한 번에 계산 후 선택 출력)
    # 1) 일별 DAU
    daily = (
    ...
    # 2) 7일 윈도우 WAU (유저-날짜 원천에서 distinct 집계)
    if not daily.empty:
        N = 7
...
# dataframe: use_container_width=True 유지 (width 문자열 사용 금지)
st.dataframe(
